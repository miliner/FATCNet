...
class mIoU():
    ...
    def get(self):
        ...
        mIoU = IoU
        # pixAcc = np.sum(self.pixAcc)/len(self.pixAcc)
        # mIoU = np.sum(self.miou)/len(self.miou)
        return pixAcc, mIoU
...
class ROCMetric():
    # Receiver Operation Characteristics：描述不同Fa下Pd的变化趋势
    ...
    def __init__(self, nclass, bins):  # bin的意义实际上是确定ROC曲线上的threshold取多少个离散值
        ...
        self.bins = bins  # ROC曲线上的阈值取多少个离散值
...
def cal_tp_pos_fp_neg(predict, target, nclass, score_thresh):
    predict = (predict > score_thresh).float()
    # #######################
    # predict = (torch.sigmoid(predict) > score_thresh).float()
...
class PD_FA():
    ...
    def update(self, preds, labels):
        ...
        for iBin in range(self.bins + 1):
            ...
            # measure.label()实现连通区域标记
            # connectivity：None则默认是取最高值，对于二维来说，当connectivity=1时代表4连通，当connectivity=2时代表8连通.
            image = measure.label(predits, connectivity=2)
            # measure.regionprops()测量标记图像区域的属性
